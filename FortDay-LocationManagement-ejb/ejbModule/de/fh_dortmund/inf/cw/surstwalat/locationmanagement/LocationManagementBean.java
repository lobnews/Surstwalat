...
public class LocationManagementBean implements LocationManagementLocal {
	...
	private boolean checkForCollisionWithPlayer(Playground playground, Token token, int pos, int count) {
		...
		if (playground.getField(pos + count).getToken() != null) {
			...
			if (token.getPlayerId() == enemy.getPlayerId()) {
				outgoingEvents.triggerCollisionWithOwnCharacterMessage(playground.getGame().getId(),
						token.getPlayerId(), token.getNr());
				return true;

			} else {
				outgoingEvents.triggerCollisionWithPlayerMessage(playground.getGame().getId(), token.getPlayerId(),
						token.getNr(), enemy.getPlayerId(), enemy.getNr());
				return false;
			}
		}
	...
	private boolean checkForCollisionWithItem(Playground playground, Token token, int pos, int count) {
		...
		if (playground.getField(pos + count).getItem() != null) {
			outgoingEvents.triggerCollisionWithItemMessage(playground.getGame().getId(), token.getPlayerId(),
					token.getNr(), playground.getField(pos + count).getItem().getId());
